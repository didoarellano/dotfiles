...
function! SyntaxCheckers_php_GetLocList()
    ...
    " Sticking with basic php -l errors for now. phpcs enforces the PEAR style
    " guide. I don't use php enough to follow a strict style guide.
    " if empty(errors)
...
function! s:GetPHPCSErrors()
    let makeprg = "php ~/Code/third-party/PHP_CodeSniffer/scripts/phpcs " . g:syntastic_phpcs_conf . " --report=csv ".shellescape(expand('%'))
